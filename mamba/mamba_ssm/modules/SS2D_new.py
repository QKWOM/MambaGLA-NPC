...
class SS2D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        x_stack = torch.stack([X_HWD,X_local],dim=1).view(B,2,-1,L)
        ...
        wh_y = out_y[:,0].view(B,-1,D,W,H).permute(0,1,4,2,3).contiguous().view(B,-1,L)
        invwh_y = inv_y[:,0].view(B,-1,D,W,H).permute(0,1,4,2,3).contiguous().view(B,-1,L)

        local_y = out_y[:,1].view(B,-1, Dg, w, Hg, w, Wg, w).permute(0,1,2,5,3,6,4,7).contiguous().view(B,-1,L)
        invlocal_y = inv_y[:,1].view(B,-1, Dg, w, Hg, w, Wg, w).permute(0,1,2,5,3,6,4,7).contiguous().view(B,-1,L)
        # print(local_y.shape)
        # print(out_y[:,0].shape)
        return wh_y, invwh_y, local_y, invlocal_y
...
class SS_Conv_SSM(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        # print('hzh',input_right.shape)
        # input_left, input_right = input, input
        # input_left = input_left.permute(0,2,3,4,1)
        # input_right = input_right.permute(0,2,3,4,1)
        ...
        input_left = input_left.permute(0, 2, 3, 4, 1).contiguous()
