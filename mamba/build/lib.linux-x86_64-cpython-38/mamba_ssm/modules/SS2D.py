...
class SS2D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        x_stack = torch.stack([X_DHW,X_HWD,X_local],dim=1).view(B,3,-1,L)
        ...
        wh_y = out_y[:,1].view(B,-1,D,W,H).permute(0,1,4,2,3).contiguous().view(B,-1,L)
        invwh_y = inv_y[:,1].view(B,-1,D,W,H).permute(0,1,4,2,3).contiguous().view(B,-1,L)

        local_y = out_y[:,2].view(B,-1, Dg, w, Hg, w, Wg, w).permute(0,1,2,5,3,6,4,7).contiguous().view(B,-1,L)
        invlocal_y = inv_y[:,2].view(B,-1, Dg, w, Hg, w, Wg, w).permute(0,1,2,5,3,6,4,7).contiguous().view(B,-1,L)
        # print(local_y.shape)
        # print(out_y[:,0].shape)
        return out_y[:, 0], inv_y[:, 0], wh_y, invwh_y, local_y, invlocal_y
