...
class SegMamba(nn.Module):
    def __init__(
        ...
        self.vit = VSSLayer(
                    dim=768,
                    depth=8,
                    d_state=math.ceil(768 / 6),  # 20240109
                    drop=0.0,
                    attn_drop=0,
                    drop_path=0.,
                    norm_layer=nn.LayerNorm,
    ...
    def forward(self, x_in):
        ...
        x2 = self.downsample_layers[0](x_in)  # 48xD/2xH/2xW/2
        enc2 = self.encoder2(x2)  # 96xD/2xH/2xW/2

        x3 = self.downsample_layers[1](x2)  # 96xD/4xH/4xW/4
        enc3 = self.encoder3(x3)  # 192xD/4xH/4xW/4

        x4 = self.downsample_layers[2](x3)  # 192xD/8xH/8xW/8
        enc4 = self.encoder4(x4)  # 384xD/8xH/8xW/8

        x5 = self.downsample_layers[3](x4)  # 384xD/16xH/16xW/16
        # print(x5.shape)
        enc_hidden = self.encoder5(x5)  # 768xD/16xH/16xW/16
        enc_hidden = self.vit(enc_hidden)
