...
class SegMamba(nn.Module):
    def __init__(
        ...
        self.out = UnetOutBlock(spatial_dims=spatial_dims, in_channels=feat_size[0]//2, out_channels=self.out_chans)

        # self.center = UnetConv3(filters[3], filters[4], self.is_batchnorm)
        self.gating4 = UnetGridGatingSignal3(feat_size[3], feat_size[2], kernel_size=(1, 1, 1), is_batchnorm=True)
        self.gating3 = UnetGridGatingSignal3(feat_size[2], feat_size[1], kernel_size=(1, 1, 1), is_batchnorm=True)
        self.gating2 = UnetGridGatingSignal3(feat_size[1], feat_size[0], kernel_size=(1, 1, 1), is_batchnorm=True)
        self.gating1 = UnetGridGatingSignal3(feat_size[0], feat_size[0]//2, kernel_size=(1, 1, 1), is_batchnorm=True)

        # attention blocks
        self.attentionblock1 = GridAttentionBlock3D(in_channels=feat_size[0]//2, gating_channels=feat_size[0]//2,
                                                    inter_channels=feat_size[0]//2, sub_sample_factor=(2, 2, 2),
                                                    mode=mode)
    ...
    def forward(self, x_in):
        outs = self.vit(x_in)
        # print(x_in.shape)
        enc1 = self.encoder1(x_in)# 24xDxHxW
        # print(enc1.shape)
        ...
        gating4 = self.gating4(x5) # (1,192,8,8,8)

        g_enc4 = self.attentionblock4(x4, gating4) # (1,192,16,16,16)
        # enc_hidden = self.global_avg_pool_and_sigmoid(enc_hidden)
        dec3 = self.decoder5(x5, g_enc4) # (1,192,16,16,16)


        gating3 = self.gating3(dec3) # (1,96,16,16,16)

        g_enc3 = self.attentionblock3(x3, gating3)  # (1,96,32,32,32)
        # dec3 = self.global_avg_pool_and_sigmoid(dec3)
        dec2 = self.decoder4(dec3, g_enc3) # (1,96,32,32,32)


        gating2 = self.gating2(dec2) # (1,48,32,32,32)

        g_enc2 = self.attentionblock2(x2, gating2)  # (1,48,64,64,64)
        # dec2 = self.global_max_pool_and_sigmoid(dec2)
        dec1 = self.decoder3(dec2, g_enc2) # (1,48,64,64,64)


        gating1 = self.gating1(dec1) # (1,24,64,64,64)

        g_enc1 = self.attentionblock1(enc1, gating1)  # (1,24,128,128,128)
        # dec1 = self.global_max_pool_and_sigmoid(dec1)
        dec0 = self.decoder2(dec1, g_enc1) # (1,24,128,128,128)


        # dec0 = self.global_max_pool_and_sigmoid(dec0)
        out = self.decoder1(dec0)  # (1,24,128,128,128)
