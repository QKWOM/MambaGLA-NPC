...
class MambaEncoder(nn.Module):
    def  __init__(self, in_chans=1, depths=[2, 2, 2, 2], dims=[48, 96, 192, 384], d_state=16,
        ...
        stem = nn.Sequential(
            ...
            # nn.InstanceNorm3d(dims[0], eps=1e-6)
        )
        ...
        for i in range(4):
            ...
            stage = nn.Sequential(
                # nn.InstanceNorm3d(dims[i], eps=1e-6),
        ...
        # norm_layer = partial(nn.InstanceNorm3d, eps=1e-6)
    ...
    def forward_features(self, x):
        ...
        for i in range(4):
            ...
            # x,mamba_out,cnn_out = self.stages[i](x)
            x = self.stages[i](x)
            ...
            if i in self.out_indices:
                ...
                outs.append(x_out)
                x = x_out
        return tuple(outs)#, tuple(mamba_outs), tuple(cnn_outs)
...
class SegMamba(nn.Module):
    def __init__(
            ...
            mode='enhanced_grid_and_channel',
        ...
        self.encoder_modules = [
            self.vit, self.encoder1, self.encoder2, self.encoder3,
        ...
        self.decoder_modules = [
            ...
            self.decoder4, self.decoder5, self.out,
            self.gating4, self.gating3, self.gating2, self.gating1,
            self.attentionblock1, self.attentionblock2, self.attentionblock3, self.attentionblock4
        ]
    ...
    def forward(self, x_in):
        # enc_list = []
        # g_enc_list = []
        # outs,mamba,cnn = self.vit(x_in)
        x_in = x_in.to(self.device1)
        outs = self.vit(x_in)

        enc1 = self.encoder1(x_in)  # 48xDxHxW

        x2 = outs[0].to(self.device1)  # 48xD/2xH/2xW/2
        enc2 = self.encoder2(x2)  # 96xD/2xH/2xW/2

        x3 = outs[1].to(self.device1)  # 96xD/4xH/4xW/4
        enc3 = self.encoder3(x3)  # 192xD/4xH/4xW/4

        x4 = outs[2].to(self.device1)  # 192xD/8xH/8xW/8
        enc4 = self.encoder4(x4)  # 384xD/8xH/8xW/8

        x5 = outs[3].to(self.device1)  # 384xD/16xH/16xW/16
        enc_hidden = self.encoder5(x5)  # 768xD/16xH/16xW/16

        enc_hidden = enc_hidden.to(self.device2)
        enc4 = enc4.to(self.device2)
        enc3 = enc3.to(self.device2)
        enc2 = enc2.to(self.device2)
        enc1 = enc1.to(self.device2)

        gating4 = self.gating4(enc_hidden) # (1,384,8,8,8)

        g_enc4 = self.attentionblock4(enc4, gating4) # (1,384,16,16,16)
        enc_hidden = self.global_max_pool_and_sigmoid(enc_hidden)
        ...
        dec3 = self.global_max_pool_and_sigmoid(dec3)
        ...
        result = self.out(out)
        # enc_list.append(enc1)
        # enc_list.append(enc2)
        # enc_list.append(enc3)
        # enc_list.append(enc4)
        # g_enc_list.append(g_enc1)
        # g_enc_list.append(g_enc2)
        # g_enc_list.append(g_enc3)
        # g_enc_list.append(g_enc4)
        return result#,mamba,cnn,enc_list,g_enc_list
