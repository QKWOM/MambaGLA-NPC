...
from mamba.mamba_ssm.modules.local_vertical_attention import VSSLayer
import math
...
class MambaEncoder(nn.Module):
    def  __init__(self, in_chans=1, depths=[2, 2, 2, 2], dims=[48, 96, 192, 384], d_state=16,
                 drop_path_rate=0.0, layer_scale_init_value=1e-6, out_indices=[0, 1, 2, 3], fft_nums = [0,0,2,2], in_shape=[256,256,256]):
        super().__init__()
        ...
        for i in range(4):
            ...
            # stage = nn.Sequential(
            #     *[Spectral_Layer(dim=dims[i], stage=i + 1, in_shape=in_shape)
            #       if j < fft_nums[i]
            #       else VSSLayer(
            #             dim=dims[i],
            #             depth=depths[i],
...
class PointPromptEncoder(nn.Module):
    def __init__(self, hidden_size, num_points=6, point_feature_dim=32):
        super().__init__()
        ...
        self.coord_encoder = nn.Sequential(
            nn.Linear(3, point_feature_dim // 2),
            nn.ReLU(),
            nn.Linear(point_feature_dim // 2, point_feature_dim)
        )

        # 点类型编码（前景/背景）
        self.type_encoder = nn.Embedding(2, point_feature_dim // 2)

        # 融合层
        self.fusion = nn.Sequential(
            nn.Linear(point_feature_dim + point_feature_dim // 2, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, hidden_size)
        )

    def forward(self, points_coords, points_labels, spatial_size):
        """
        points_coords: [B, num_points, 3] - 归一化的坐标 [0,1]
        points_labels: [B, num_points] - 0:背景, 1:前景
        spatial_size: 目标空间尺寸 [D, H, W]
        """
        B, num_points, _ = points_coords.shape
        D, H, W = spatial_size

        # 编码坐标信息
        coords_features = self.coord_encoder(points_coords)  # [B, num_points, point_feature_dim]

        # 编码类型信息
        type_features = self.type_encoder(points_labels.long())  # [B, num_points, point_feature_dim//2]

        # 融合坐标和类型信息
        point_features = torch.cat([coords_features, type_features], dim=-1)
        point_features = self.fusion(point_features)  # [B, num_points, hidden_size]

        # 将点特征映射到空间位置
        spatial_point_features = torch.zeros(B, self.hidden_size, D, H, W).to(points_coords.device)

        # 将归一化坐标转换为实际坐标
        actual_coords = (points_coords * torch.tensor([D - 1, H - 1, W - 1]).to(points_coords.device)).long()

        for b in range(B):
            for p in range(num_points):
                d, h, w = actual_coords[b, p]
                # 确保坐标在有效范围内
                d = torch.clamp(d, 0, D - 1)
                h = torch.clamp(h, 0, H - 1)
                w = torch.clamp(w, 0, W - 1)

                spatial_point_features[b, :, d, h, w] += point_features[b, p]

        return spatial_point_features
...
class SegMamba(nn.Module):
    def __init__(
            ...
            num_points=6,  # 添加点数量参数
    ) -> None:
        super().__init__()

        # 添加点提示编码器
        self.point_prompt_encoder = PointPromptEncoder(
            hidden_size=feat_size[0],  # 使用第一个特征图的通道数
            num_points=num_points,
            point_feature_dim=32
        )

        # 添加点特征融合卷积
        self.point_fusion_conv = nn.Sequential(
            nn.Conv3d(feat_size[0] * 2, feat_size[0], 3, padding=1),
            nn.BatchNorm3d(feat_size[0]),
        ...
        self.gating4 = UnetGridGatingSignal3(768, 384, kernel_size=(3, 3, 3), is_batchnorm=True)
        self.gating3 = UnetGridGatingSignal3(384, 192, kernel_size=(3, 3, 3), is_batchnorm=True)
        self.gating2 = UnetGridGatingSignal3(192, 96, kernel_size=(3, 3, 3), is_batchnorm=True)
        self.gating1 = UnetGridGatingSignal3(96, 48, kernel_size=(3, 3, 3), is_batchnorm=True)
    ...
    def forward(self, x_in, points_coords=None, points_labels=None):
        B, C, D, H, W = x_in.shape

        # 如果没有提供点提示，使用默认值（无点）
        if points_coords is None:
            points_coords = torch.zeros(B, 6, 3).to(x_in.device)
            points_labels = torch.zeros(B, 6).to(x_in.device)

        # 编码点提示
        point_features = self.point_prompt_encoder(
            points_coords, points_labels, [D, H, W]
        )  # [B, feat_size[0], D, H, W]
        ...
        # 融合点特征到第一个编码器输出
        enc1_with_points = torch.cat([enc1, point_features], dim=1)
        enc1 = self.point_fusion_conv(enc1_with_points)
